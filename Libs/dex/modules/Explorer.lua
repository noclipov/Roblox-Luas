...
local function main()
	...
	Explorer.ShowRightClick = function(MousePos)
		...
		if presentClasses["Player"] then
			context:AddRegistered("TELEPORT_TO_PLAYER")
	...
	Explorer.InitRightClick = function()
		...
		context:Register("TELEPORT_TO_PLAYER",{Name = "Teleport to", IconMap = Explorer.MiscIcons, Icon = "TeleportTo", OnClick = function()
			local sList = selection.List
			local plrRP = plr.Character and plr.Character:FindFirstChild("HumanoidRootPart")
			...
			for _,node in next, sList do
				local Obj = node.Obj
				if Obj:IsA("Player") and Obj.Character then Obj = Obj.Character.PrimaryPart end
